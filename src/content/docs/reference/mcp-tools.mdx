...
From a shell, the `chorus mcp call <tool>` command calls any of these tools with the same
agent credential — a token-free path for scripts and one-off calls. See
[The chorus CLI](/reference/cli/).
